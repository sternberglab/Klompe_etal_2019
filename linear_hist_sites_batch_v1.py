py_name = 'linear_hist_sites_batch_v1.py'
...
norm_on = False  # normalize each data point against total (genome mapped) reads in each sample (as percentage)
...
def plot(filename, bin_size, date, desc, psl, spacer_location, norm_on):  # main graphing function
    ...
    # save figures
    plt.savefig('plots\\TEST_{}_{}_{}.svg'.format(date, code, desc), dpi=500)  # check folders
...
with open('input.csv', newline='') as csvfile:
    ...
    for row in reader:
        ...
        filename = 'none'
        for i in os.listdir('.'):
            if fnmatch.fnmatch(i, "TEST{}*{}.csv".format(code, mode)):
                filename = i
                break
        if filename != 'none':
            desc = row[6]
